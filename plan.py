...
from paths import save_dir, plan_data_path, check_uptodate, dict_dir
from random import random, shuffle
from rank_words import RankedWords
from singleton import Singleton
from utils import split_sentences, NUM_OF_SENTENCES, is_cn_char
import jieba
import os
import re
...
class Planner(Singleton):
    ...
    def __init__(self):
        ...
        if not os.path.exists(_plan_model_path):
            train_planner()
        #self.model = models.FastText.load(_plan_model_path)
    ...
    def _expand(self, keywords):
        if len(keywords) < NUM_OF_SENTENCES + 1:
            ...
            word_idx = 0
            s = 0
            while len(keywords) < (NUM_OF_SENTENCES + 1) \
...
if __name__ == '__main__':
    ...
    #keywords = planner.plan("奥巴马")
    #keywords = planner.plan("秋天来了，大雁飞过")
    print(keywords)
